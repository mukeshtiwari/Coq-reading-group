...
Proof.
  intros. unfold t_update.
  apply beq_string_false_iff in H.
  rewrite H. auto.
Qed.
...
Proof.
  ...
  destruct (string_dec x x0) as [H | H].
...
Proof.
  ...
  destruct (string_dec x x1) as [H | H].
  rewrite H. rewrite <- beq_string_refl. auto.
  apply beq_string_false_iff in H. rewrite H. auto.
Qed.
...
Proof.
  ...
  unfold t_update.
  apply functional_extensionality; intros.
  destruct (string_dec x1 x) as [H1 | H1];
    destruct (string_dec x2 x) as [H2 | H2].
  subst. intuition.
  rewrite H1. rewrite <- beq_string_refl.
  apply beq_string_false_iff in H2. rewrite H2. auto.
  rewrite H2. rewrite <- beq_string_refl.
  apply beq_string_false_iff in H1. rewrite H1. auto.
  apply beq_string_false_iff in H1. apply beq_string_false_iff in  H2.
  rewrite H1, H2. auto.
Qed.
